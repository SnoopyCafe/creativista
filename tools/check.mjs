...
const imagesWithoutAlt = imgTags.filter((tag) => {
  ...
  return alt === null || (!alt.trim() && attr(tag, 'aria-hidden') !== 'true');
});
assert('every informative <img> has non-empty alt', imgTags.length > 0 && imagesWithoutAlt.length === 0, `${imgTags.length} images checked; ${imagesWithoutAlt.length} missing/empty without aria-hidden`);
...
const email = 'admin@creativistapods.com';
const emailAnchors = visibleHtml.match(/<a\b[^>]*href=["']mailto:admin@creativistapods\.com["'][^>]*>[\s\S]*?<\/a>/gi) ?? [];
const visibleEmailCopies = visibleHtml.match(/admin@creativistapods\.com/gi) ?? [];
const linkedEmailOccurrences = emailAnchors.reduce((sum, anchor) => sum + (anchor.match(/admin@creativistapods\.com/gi) ?? []).length, 0);
